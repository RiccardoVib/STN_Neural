...
class MyLRScheduler(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __call__(self, step):
        ...
        return tf.math.maximum(lr, 1e-7)
...
def render_results(pred, S_v, rms_v, alfas_v, attackTimes_v, model_save_dir, save_folder):

    if S_v is not None:
        predictions = np.array(pred[2].reshape(-1))
        predictWaves(predictions, np.array(S_v, dtype=np.float32), model_save_dir, save_folder, 24000, 'S')
    if rms_v is not None:
        predictions = np.array(pred[3].reshape(-1))
        plotResult_(predictions, rms_v, model_save_dir, save_folder, 'rms')
    # if alfas_v is not None:
    #     predictions = np.array(pred[4].reshape(-1))
    #     plotResult_(predictions, alfas_v, model_save_dir, save_folder, 'alfa')
    # if attackTimes_v is not None:
    #     predictions = np.array(pred[5].reshape(-1))
    #     plotResult_(predictions, attackTimes_v, model_save_dir, save_folder, 'attack')