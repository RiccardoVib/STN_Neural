...
class STFT_N_loss(tf.keras.losses.Loss):
    def __init__(self, m=[32, 64, 128, 256, 512, 1024], name="STFT", **kwargs):
        super().__init__(name=name, **kwargs)
        self.m = m
    ...
    def call(self, y_true, y_pred):
        ...
        for i in range(len(self.m)):
            ...
            loss += tf.divide(tf.norm((l_true - l_pred), ord=1), tf.norm(l_true, ord=1) + 1e-9) + \
                    tf.divide(tf.norm((Y_true - Y_pred), ord=1), tf.norm(Y_true, ord=1) + 1e-9)
